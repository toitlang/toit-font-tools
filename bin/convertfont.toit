...
main args:
  root-cmd := cli.Command "convert"
    --help="Convert a BDF font to a Toit font."
    --options=[
        cli.Flag "bold"
            --short-name="b"
...
class FontReader:
  ...
  dump out-filename copyright-file -> none:
    ...
    writer := fd.out
    copyright-lines.do:
      writer.write "// Copyright: $it\n"
    if copyright-file:
      dump-copyright-file copyright-file writer
    writer.write "\n"
    if comment-lines.size != 0:
      if doc-comments:
        writer.write "/**\n"
      comment-lines.do:
        writer.write "$(doc-comments ? "" : "// ")$it\n"
      if doc-comments:
        writer.write "*/\n"
      writer.write "\n"
    writer.write "/// Bitmaps for the $font-name font\n"
    writer.write "\n"
    short-filename := filename
    slash := short-filename.index-of --last "/"
    if slash != -1:
      short-filename = filename.copy slash + 1
    writer.write "// Autogenerated by convertfont.toit from the BDF file $short-filename\n"
    if bold:
      writer.write "// This font was automatically made bold by smearing the pixels horizontally\n"

    if (blocks.any: it.used and it.unicode-block.contains-icons):
      writer.write "\n"
      writer.write "import font show Font\n"
      writer.write "import icons show Icon\n"
    ...
    blocks.do: | block |
      if block.used
        ...
        if not block.unicode-block.contains-icons:
          write-block-intro_ writer block block-size
        name-bytes := ByteArray block.name.size
        name := block.all-caps-identifier
        if block.unicode-block.contains-icons:
          writer.write "$(name) ::= Font.from_page_ #[\n"
        else:
          writer.write "$(name) ::= #[\n"

        (CArrayDumper block counter).dump fd

        dump-char-names block writer

    fd.close

  write-block-intro_ writer/io.Writer block/Block block-size/int -> none:
    writer.write "/**\n"
    writer.write "The characters from the $block.name Unicode block in the $font-name font.\n"
    writer.write "  (See https://en.wikipedia.org/wiki/$block.unicode-block.wikipedia-name )\n"
    present-count := 0
    block.do: present-count++
    assigned := block.unicode-block.assigned
    if present-count >= assigned:
      writer.write "This block has $assigned assigned code points, and they are all\n"
      writer.write "  present in this font.\n"
    else:
      list-them := present-count <= 14
      snippet := present-count > assigned / 2 ? "and" : "but only"
      is-are := present-count == 1 ? "is" : "are"
      writer.write "This block has $assigned assigned code points, $snippet $present-count of\n"
      writer.write "  them $is-are present in this font$(list-them ? ":" : ".")\n"
      i := 0
      if list-them:
        block.do: | c code-point |
          ...
          writer.write "  $name$comma$and-word\n"
    writer.write "  This block contains characters in the range 0x$(%04x block.from)-0x$(%04x block.to).\n"
    writer.write "  The bitmaps for this block in this font take up about $block-size bytes.\n"
    writer.write "*/\n"

  dump-char-names block/Block writer/io.Writer -> none:
    ...
    block.do: | c/Char encoding/int |
      if c.name and c.name != "":
        ...
        writer.write "$character-name ::= Icon 0x$(%x encoding) $block-identifier\n"
